 ...
 *  2. download a specific version/zip package from
 * https://gitea.swerk.priv.at/stefan/-/packages/composer/stefan%2Fgtn_jku_similarity_comparison/0.0.2
...
Logger::debug("block_exaquest_similarity_comparison - passed params: ",
    ["courseid" => $courseID, "action" => $action, "sort" => $sortBy]);

$courseContext = context_course::instance($courseID); // currently optional
$course = get_course($courseID); // currently optional
$allCourseCategories =
    core_course_category::get_all($options = ["returnhidden"]); // retrieve all course categories, currently optional

$recurse = $pagevars["recurse"];
$categoryList = getQuestionCategories($pagevars, $recurse); // retrieve sub categories if recurse is set

$moodleQuestions = loadMoodleQuestions($categoryList, $examid); // core Moodle DB question records (arr of stdClass objects)
Logger::debug("block_exaquest_similarity_comparison - question course context: ",
    ["qCategories" => $categoryList, "nrOfMoodleQuestions" => count($moodleQuestions)]);
...
function validateInput(string $param, string $value): string {
    ...
    switch ($param) {
        case 'action':
            ...
            Logger::debug("block_exaquest_similarity_comparison - unknown action: ",
                ["action" => $value, "knownActions" => $allowedActions]);
            break;
        case 'sort':
 ...
 */
function computeSimilarityComparisonTableData(array $moodleQuestions, array $comparatorSettings,
    ?moodle_database $DB = null): array {
 ...
 * @return array
 */
function loadMoodleQuestions(array $categoryList, $examid): array {
    ...
    // examid == quizid
    if ($examid) {
        // get all questions that are added to this exam:
        /*
        ...
        JOIN mdl_question_references mqr on qbe.id = mqr.questionbankentryid
        JOIN mdl_quiz_slots qs ON qs.id = mqr.itemid
        WHERE qs.quizid = 22
        ...
        $sql = "SELECT q.*,
              ...
              JOIN {quiz_slots} qzs
                ON qzs.id = qr.itemid
              WHERE qzs.quizid = :quizid
              AND qs.status = " . BLOCK_EXAQUEST_QUESTIONSTATUS_RELEASED . " AND qc.id = :questioncategoryid
              AND qv.version = (SELECT MAX(v.version)
                                                     ...
                                                     WHERE be.id = qbe.id)";
        // Load the questions.
        return $DB->get_records_sql($sql, array('questioncategoryid' => $catAndCont[0], 'quizid' => (int)$examid));
    } else {
        $sql = "SELECT q.*,
                   qc.id as category,
                   qv.status,
              ...
              AND qv.version = (SELECT MAX(v.version)
                                                     ...
                                                     WHERE be.id = qbe.id)";
        // Load the questions.
        return $DB->get_records_sql($sql, array('questioncategoryid' => $catAndCont[0]));
    }
}
 ...
 * @return bool false, iff $overwriteExisting is false and there is a record with matching foreign keys question_id1 and
 *     question_id2
 * @throws dml_exception
 */
function storeSimilarityComparisonResult(moodle_database $DB, stdClass $similarityRecordObjectWrapper,
    bool $overwriteExisting = true): bool {
    ...
    Logger::debug("New/updated similarity database record stored: ",
        ["ID" => $newID, "similarityRecord" => $similarityRecordObjectWrapper]);
    return true;
}
...
function getSimilarityRecord(moodle_database $DB, int $qID1, int $qID2): stdClass|bool {
    $similarityRecord =
        $DB->get_record(DB_BLOCK_EXAQUEST_SIMILARITY, ['question_id1' => $qID1, 'question_id2' => $qID2], '*', IGNORE_MISSING);
    // check reverse mapping as well, in case it does not exist
    if (!$similarityRecord) {
        $similarityRecord =
            $DB->get_record(DB_BLOCK_EXAQUEST_SIMILARITY, ['question_id1' => $qID2, 'question_id2' => $qID1], '*', IGNORE_MISSING);
    }
...
function verify_settings(array $similarityComparisonSettings): array {
    ...
    Logger::debug("block_exaquest_similarity_comparison - setting jwMinPrefixLength to " .
        $similarityComparisonSettings["jwMinPrefixLength"]);

    if (!isset($jwprefixscale) || $jwprefixscale < 0.0 ||
        $jwprefixscale > (1 / $similarityComparisonSettings["jwMinPrefixLength"])) {
    ...
    Logger::debug("block_exaquest_similarity_comparison - setting jwprefixscale to " .
        $similarityComparisonSettings["jwPrefixScale"]);
    ...
    Logger::debug("block_exaquest_similarity_comparison - setting swgMatchValue to " .
        $similarityComparisonSettings["swgMatchValue"]);
    ...
    Logger::debug("block_exaquest_similarity_comparison - setting swgMismatchValue to " .
        $similarityComparisonSettings["swgMismatchValue"]);
